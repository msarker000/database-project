...
public class DBUtil {
	 ...
	 * @param relation
	 */
	public static void findNormalForm(Relation relation) {
		NormalForm relationNormalForm = NormalForm.NONE;
		for (FunctionDependency fd : relation.getOriginalFDs()) {
			if (DBUtil.isBCNF(fd, relation.getKeys())) {
				fd.setNormalForm(NormalForm.BCNF);
			} else if (DBUtil.is3NF(fd, relation.getKeys())) {
				fd.setNormalForm(NormalForm.THIRD_NF);
			} else if (DBUtil.is2NF(fd, relation.getKeys())) {
		...
		relation.setNormalForm(relationNormalForm);
	}
	 ...
	 * @return
	 */
	public static Set<Character> findClosure(Set<FunctionDependency> functionDependencies, Set<Character> seeds) {
	 ...
	 * @param relation
	 */
	public static void findKeys(Relation relation) {
		Set<Character> allAttributesOnBothSideOfFDs = relation.getConvertedNonTrivialFDs().stream()
				.map(x -> x.getAllAttributesOfbothSide()).flatMap(x -> x.stream()).collect(Collectors.toSet());
		Set<Character> attributesOnLhsSideOfFDs = relation.getConvertedNonTrivialFDs().stream()
				.map(x -> x.getAttributesOfLhsOfFD()).flatMap(x -> x.stream()).collect(Collectors.toSet());
		Set<Character> attributesOnRhsSideOfFDs = relation.getConvertedNonTrivialFDs().stream()
		...
		Set<Character> attributesNotOnLhsOrRhsOfFDs = SetUtil.difference(relation.getAttributes(),
		...
		Set<Character> closureSet = findClosure(relation.getConvertedNonTrivialFDs(),
				attributesNotOnLhsOrRhsAndOnlyOnLhsOfFDs);
		
		if (SetUtil.isMatched(relation.getAttributes(), closureSet)) {
			...
			for (int i = 1; i <= attributesFoundOnBothSideOfFDs.size(); i++) {
				...
				for (Set<Character> characters : setofAttributes) {
					...
					if (!DBUtil.isNewAttributesSetInSuperSetOfAKey(newAttributesSets, keys)) {
						closureSet = findClosure(relation.getConvertedNonTrivialFDs(), newAttributesSets);
						if (SetUtil.isMatched(relation.getAttributes(), closureSet)) {
	 ...
	 * @return
	 */
	private static boolean isBCNF(FunctionDependency fd, Set<Set<Character>> keys) {
	 ...
	 * @return
	 */
	private static boolean is3NF(FunctionDependency fd, Set<Set<Character>> keys) {
	 ...
	 * @return
	 */
	private static boolean is2NF(FunctionDependency fd, Set<Set<Character>> keys) {
		// A relation is in 3NF if for every non-trivial FD X → A, X is a
		Set<Character> lhsOfFD = fd.getAttributesOfLhsOfFD();
