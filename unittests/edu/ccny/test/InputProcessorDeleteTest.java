...
public class InputProcessorDeleteTest {
	...
	public void testDeleteCASCADE() throws Exception {
		...
		inputProcessor.insertIntoTable("INSERT INTO Departments VALUES('PHE', '1003');");
		...
		//dbService.printTable("Departments");
		
		List<Tuple> tuples2 = inputProcessor.selectFromTable("SELECT * FROM students WHERE A >= 30  and A <= 60");
		//inputProcessor.deleteFromTable("DELETE FROM students WHERE A < 40");
		tuples2 = inputProcessor.selectFromTable("SELECT * FROM students WHERE A <= 30");
		//dbService.printTable("Students");
		//dbService.printTable("Departments");
		
		inputProcessor.deleteFromTable("DELETE FROM Departments");
	...
	@Test
	public void testDeleteOnSETNULL() throws Exception {
		DBService dbService = new DBOperator();
		InputProcessor inputProcessor = new InputProcessor(dbService);
		// department table
		inputProcessor.createTable("CREATE TABLE Departments(N string NOT NULL, C integer PRIMARY KEY);");
